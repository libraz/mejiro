...
| 4 | まとまりを保つ語の内部。既定では接続詞・副詞・連体詞・代名詞・感動詞。後述の「語のまとまりを保つ」を参照 |
...
`shortfall(p)` は、その行が行長に対してどれだけ短く終わるかを em 単位で表した値です。既定の重み（`penaltyWeight` は `1`、`shortfallWeight` は `1.5`）では、罰則 2 の位置を諦めるのは代わりの位置で空くのが 1.33em 未満のときまで、罰則 3 なら 2em 未満のときまで、最大の罰則 4 でも 2.67em 未満のときまでです。この上限こそが重みの狙いです。縦組みの日本語は文字グリッドの上に組まれ、禁則処理が行をずらすのは通常 1 文字、多くて 2 文字です。最大の罰則が 4 文字分の空きと引き換えになる重み `1` は、慣習の許す範囲を大きく超えます。効くのは 2 つの重みの比だけで、両方を同じ倍率にしてもすべてのコストが同じ倍率になるだけで順序は変わりません。片方だけ動かし、もう片方は既定のままにしてください。

窓の幅は `maxBacktrackChars`（既定 6）で、改行処理が文字数に対して線形であり続けるのはこの上限のおかげです。6 は候補が勝てなくなる境目でもあるので、窓を広げても探索時間が増えるだけです。`k` 個手前の位置は少なくとも `0.5k` em を捨てることになり（2 つの位置の間に置ける最も狭い文字が半角文字だからです）、配列中の最大の罰則を `P` とすると、勝てるのは `k < 2 * penaltyWeight * P / shortfallWeight` のあいだだけです。既定の重みと `deriveTypographyHints()` が出す罰則では `k < 5.33` になります。だからこそ 6 で探索は完全に覆われます。重みを `1` に近づけたうえで、まとまりを保つ罰則も上げる場合は、この境目が 6 を超えます。窓も併せて広げてください。そうしないと、自分の設定では勝つはずの位置に探索が届きません。窓の中に有効な位置が 1 つもない場合は第1節の範囲無制限の探索に落ちるので、行頭禁則文字が長く続く場合もこれまでどおり扱えます。
...
罰則 4 は、表のなかで唯一、構造から読み取るのではなく呼び出し側が選ぶ値です。`keepWholePos` は内部で改行したくない品詞を挙げるオプションで、該当する形態素の内部の位置には、形態素内部の通常の罰則 2 に代えて `keepWholePenalty` が与えられます。前後の境界はそれぞれの値のままです。改行が逃げていく先がそこだからです。

既定の集合は閉じたクラスの自立語で、`DEFAULT_KEEP_WHOLE_POS` として公開しています。
...
これらの語は短く、多くは仮名で書かれ、ひとまとまりとして読まれます。そのため内部で改行すると、漢語の内部での改行とは違って目につきます。`国際|連合` は読めますが、`した|がって` は読めません。閉じたクラスであるということは、解析器の辞書が最も確実に知っている語でもあるということです。

コードは形態素の `extendedPos` か `pos` のどちらかと一致すれば適用されます。`'VERB'` は動詞全体を、`'VERB_連用'` は 1 つの活用形だけを選びます。規則を止めるなら `[]` を渡し、既定を置き換えずに広げたいなら既定を展開します。追加するものとしては形式名詞が代表的です。
...
品詞を見ているのはこの規則だけで、クラスタ規則は品詞を見ません。違いは、2 種類の規則がそれぞれ何を決めているかにあります。クラスタが決めるのは許されるかどうかなので、辞書の欠けはレイアウトそのものを変えてしまいます。罰則が決めるのは好ましいかどうかだけです。辞書にない語は、結局もともと与えられるはずだった罰則のままになります。辞書が欠けて起きる最悪のことは改善が起きないことであり、別のレイアウトになることはありません。

同じ理由で、`maxHardClusterChars` がクラスタに課すような長さの上限は設けていません。長いクラスタに上限が要るのは、行に収まらないクラスタが禁則を無視する強制改行で割られてしまうからです。長い形態素に上限は要りません。払えない回避はそもそも選ばれないからです。

### まとまりを保つ罰則の選び方

この値はスイッチではなく値段です。上げるほど、対象の語の内部で改行することは減り、その代わり行末の空きは増えます。以下の数値は、日本語の散文 20,046 文字（133 段落）を、1em を 16px、重みを既定として測ったものです。この資料には対象品詞の形態素が 733 個あり、1,777 文字、全体の 8.9% を占めます。どの数値からも段落の最終行は除いてあります。最終行の空きが表しているのは本文が尽きた位置であって、探索が下した判断ではないからです。

行長 24em、改行位置はおよそ 750 か所です。

| 設定 | 対象語の内部に落ちた改行 | 平均の空き | 1.5em 以上空いた行 | 最大の空き |
|---|---|---|---|---|
| 罰則を一切使わない（クラスタのみ） | 42 か所（5.8%） | 0.058em | 1.4% | 2.00em |
| 規則を止めた場合（`keepWholePos: []`） | 15 か所（2.0%） | 0.295em | 3.4% | 2.00em |
| `keepWholePenalty: 4`（既定） | 10 か所（1.4%） | 0.317em | 4.5% | 2.00em |
| `keepWholePenalty: 5` 以上 | 0 か所（0.0%） | 0.363em | 5.9% | 3.00em |

`keepWholePenalty` が 2 の場合は、規則を止めたのと同じことになります。その位置がもともと持つ値が 2 だからです。3 の結果はどの行長でも 2 と同一でしたが、これは偶然ではなく計算どおりです。`shortfallWeight` が `1.5` のとき、罰則 1 点で買える空きは 0.67em です。全角文字 1 つは 1em ですから、1 点では改行位置を 1 文字ぶんも動かせません。つまり全角の本文では、罰則の目盛りは実質 2 刻みで動きます。0 と 1 は同じ挙動、2 と 3 も同じ挙動で、形態素内部の既定値と本当に違ってくる最初の値が 4 です。既定を 3 ではなく 4 にしてあるのはこのためです。1 文字が 0.5em の欧文の並びでは 1 点でも支払えるので、これは目盛りの法則ではなく文字グリッドの性質です。

行長 32em では次のようになります。

| 設定 | 対象語の内部に落ちた改行 | 平均の空き | 1.5em 以上空いた行 | 最大の空き |
|---|---|---|---|---|
| 罰則を一切使わない（クラスタのみ） | 24 か所（4.7%） | 0.158em | 2.6% | 2.00em |
| 規則を止めた場合（`keepWholePos: []`） | 15 か所（2.9%） | 0.326em | 6.7% | 2.00em |
| `keepWholePenalty: 4`（既定） | 4 か所（0.8%） | 0.362em | 8.8% | 2.50em |
| `keepWholePenalty: 5` 以上 | 0 か所（0.0%） | 0.369em | 9.0% | 3.50em |

この規則が効くのは狭い行長のときだけではなく、どの行長でも効きます。40em・48em・56em では、規則を止めた場合の内部での改行が 7 か所・12 か所・4 か所、既定では 3 か所・5 か所・2 か所でした。40em より 48em のほうが効きが大きいのは、改行位置が行長に比例して並ぶのではなく、本文の切れ方に従って落ちるからです。効きが落ち始めるのは 56em になってからです。

既定は安上がりではありますが、ただではありません。24em では、構造由来の罰則だけでは残る内部での改行を 3 分の 1 減らし、行の最大の空きは規則を止めた場合と同じところに留めます。32em では 15 か所のうち 11 か所を減らし、最大の空きは 2.00em から 2.50em になります。5 にすればどの行長でも残りが消えますが、その代金は 3.00em から 3.50em の空きです。文字グリッドはそれを吸収できません。
...
`BookOptions.keepWholePos` と `BookOptions.keepWholePenalty` は、そのまま `deriveTypographyHints()` に渡ります。参照されるのは、罰則を出す唯一の段階である `'full'` のときだけです。
