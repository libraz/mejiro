...
/** All UI strings rendered by the bundled Vue components. */
...
export const MejiroI18nProvider = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    provide(
      ...
      computed(() => {
        const base =
          props.locale != null ? CATALOGS[props.locale] : (provided?.value ?? enMessages);
...
export function useI18n(options: UseI18nOptions = {}): ComputedRef<MejiroMessages> {
  ...
  return computed(() => {
    ...
    const base =
      options.locale != null ? CATALOGS[options.locale] : (provided?.value ?? enMessages);
    return options.messages ? { ...base, ...options.messages } : base;
  });
}
