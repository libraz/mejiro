# Vue 3 example — bookshelf gallery
...
- **`useEpub` composable** is used as a stateless loader (parses + tracks
  loading); the parsed `EpubBook` instances are folded into the app's own
  `shelf` array so the gallery persists across switches.
- **`epub` prop** on `MejiroReader` accepts the pre-parsed book — no second
  parse, instant render.
- **`logo` slot** is reused for the back button: header stays consistent.
...
```bash
yarn install
yarn workspace @mejiro/example-vue-shelf dev
```
